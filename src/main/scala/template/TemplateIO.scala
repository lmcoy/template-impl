...
import template.TemplateIO.{Text, Token, Variable}

import scala.annotation.tailrec

class TemplateIO(tokens: List[Token]) extends Template {
...
object TemplateIO {
  ...
  def getIO(reader: Reader): IO[TemplateIO] =
    for {
      tokens <- tokenize(reader)
    } yield new TemplateIO(tokens)


  private def tokenize(reader: Reader): IO[List[Token]] = {
